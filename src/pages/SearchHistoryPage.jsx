...
const SearchHistoryPage = () => {
	...
	useEffect(() => {
		const getSearchHistory = async () => {
			try {
				const res = await axios.get(`https://backend-dun-iota.vercel.app/api/v1/search/history`);
	...
	const handleDelete = async (entry) => {
		try {
			await axios.delete(`https://backend-dun-iota.vercel.app/api/v1/search/history/${entry.id}`);
