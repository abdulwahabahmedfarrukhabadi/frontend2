...
const SearchHistoryPage = () => {
	...
	useEffect(() => {
		const getSearchHistory = async () => {
			try {
				const res = await axios.get(`https://backend123-five.vercel.app/api/v1/search/history`);
	...
	const handleDelete = async (entry) => {
		try {
			await axios.delete(`https://backend123-five.vercel.app/api/v1/search/history/${entry.id}`);
